<div class="dropdown">
    ...
    <div class="dropdown-menu" aria-labelledby="CustomdropdownMenuButton1">
        ...
        <a class="dropdown-item" href="{{route('platecolor.edit',$row->id)}}"><i class="feather icon-edit mr-2"></i>{{ __("Edit") }}</a>
        @endcan
...
<div class="modal fade bd-example-modal-sm" id="delete1{{ $row->id}}" tabindex="-1" role="dialog" aria-hidden="true">
  <div class="modal-dialog modal-sm">
      <div class="modal-content">
          ...
          <p>{{__("Do you really want to delete this Plate Color")}} <b>{{ $row->plate_code }}</b>{{ __("? This process cannot be undone.") }}</p>
          </div>
          <div class="modal-footer">
              <form method="post" action="{{route('platecolor.destroy',$row->id)}}" class="pull-right">
              {{csrf_field()}}
              {{method_field("DELETE")}}
