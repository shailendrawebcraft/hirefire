...
<form id="demo-form2" method="post" enctype="multipart/form-data" @if(!empty($products))
  ...
  <div class="row">
    ...
    <div class="col-md-4">
    <div class="form-group">
      <label class="text-dark" for="hire">{{ __('Hire :') }}</label><br>
      <label class="switch">
        <input class="slider" type="checkbox" name="hire"  {{ $products->hire == '1' ? 'checked' : '' }}/>
        <span class="knob"></span>
      </label><br>
      <small>{{ __('(Please Choose Hire Available On This Product or Not)') }}</small>
    </div>
    </div>
